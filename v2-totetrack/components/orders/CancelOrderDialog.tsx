...
// element. Applied to the confirm button on mount so the salesperson sees
// a visible cue that this is a destructive action.
  ...
  return (
    ...
    >
      <div className="space-y-2">
        <h2 id="cancel-order-title" className="text-lg font-semibold text-foreground">
          Cancel order {poNumber}?
        </h2>
        <p className="text-sm text-muted-foreground">
          This marks the order as cancelled. You can still view it in the Cancelled filter,
...
/**
 * Two-step cancellation confirmation for an order. Renders an overlay modal
 * over the order detail panel; the confirm button shakes briefly on mount
 * and the user must click "Yes, Cancel" explicitly. `onConfirm` returns
 * either success or a user-facing error (shown inline when it rejects).
 */
