...
**T1. Install PostgreSQL 17 locally**
Create database `toteops_demo`. Verify connection.

**T2. Create the repo**
`git init` in `tote-platform/`, create public GitHub repo under `swarnimb`, first commit.

**T3. Copy both codebases**
...
**T7. Build the demo fixture layer**
Replace Supabase reads with seeded JSON. Block all writes behind a read-only modal. Bypass `middleware.ts` auth. Everything gated on `NEXT_PUBLIC_DEMO_MODE` so the prod code path is unchanged.

**T8. Seed the v2 fixtures**
Customers, contacts, orders, leads, lead notes, invoices, support tickets, attachments — same seed data standard.

**T9. Static export config**
`next.config.demo.mjs` — `output: 'export'`, `basePath: '/tote-platform'`, `images.unoptimized`, `trailingSlash`.

**T10. Deploy to GitHub Pages**
`deploy-demo.yml`. Verify live: no 404s on basePath assets, no console errors, every route renders.

**T11. Screenshot every v2 screen**
Dashboard, customers, orders, leads, invoices, calendar, support. Same widths as T6.

### Phase 4 — Assets

**T12. Build before/after pairs**
